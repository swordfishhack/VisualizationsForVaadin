...
public class VBarChartImage extends VVisualizationWidget {

	/** Set the tagname used to statically resolve widget from UIDL. */
	public static final String TAGNAME = "barchartimage";
	...
	public VBarChartImage() {
		...
		VisualizationUtils.loadVisualizationApi(new APILoadCallback(),
				"imagebarchart");
		ApplicationConnection.getConsole().log("VBarChartImage init");
	...
	protected void drawChart(AbstractDataTable data, AbstractDrawOptions options) {
		ApplicationConnection.getConsole().log(
				"VBarChartImage draw " + data.getNumberOfRows()
						+ " with options as follows" + options.toString());
		if (chart != null) {
			remove(chart);
		}
		
		chart = new ImageBarChart(data, (Options) options);
		//chart.addSelectHandler(new VisualizationSelectHandler(chart));
		
		VBarChartImage.this.add(chart);
	}
