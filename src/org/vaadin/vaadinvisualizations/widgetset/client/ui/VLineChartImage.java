...
public class VLineChartImage extends VVisualizationWidget {

	/** Set the tagname used to statically resolve widget from UIDL. */
	public static final String TAGNAME = "linechartimage";
	...
	/** */
	ImageLineChart chart;

	/**
	 * The constructor should first call super() to initialize the component and
	 * then handle any initialization relevant to Vaadin.
	 */
	public VLineChartImage() {
		...
		VisualizationUtils.loadVisualizationApi(new APILoadCallback(),
				"imagelinechart");
		ApplicationConnection.getConsole().log("VLineChartImage init");
	...
	protected void drawChart(AbstractDataTable data, AbstractDrawOptions options) {
		ApplicationConnection.getConsole().log(
				"VLineChartImage draw " + data.getNumberOfRows()
						+ " with options as follows" + options.toString());
		if (chart != null) {
			remove(chart);
		}
		
		chart = new ImageLineChart(data, (Options) options);
		//chart.addSelectHandler(new VisualizationSelectHandler(chart));
		
		VLineChartImage.this.add(chart);
	}
