...
public class VIntensityMap extends VVisualizationWidget {

	/** Set the tagname used to statically resolve widget from UIDL. */
	public static final String TAGNAME = "intensitymap";
	...
	public VIntensityMap() {
		...
		ApplicationConnection.getConsole().log("VIntensityMap init");
	...
	protected void drawChart(AbstractDataTable data, AbstractDrawOptions options) {
		ApplicationConnection.getConsole().log(
				"VIntensityMap draw " + data.getNumberOfRows()
						+ " with options as follows" + options.toString());
		if (chart != null) {
			remove(chart);
		}
		
		chart = new IntensityMap(data, (Options) options);
		chart.addSelectHandler(new VisualizationSelectHandler(chart));
		
		VIntensityMap.this.add(chart);
	}
